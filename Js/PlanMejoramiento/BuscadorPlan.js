$('#buscador').keyup(function (e) {
    ...
    $.post('../../Controller/PlanMejoramiento/Buscar.C.php', data, function (respuesta) {
        ...
        datos.forEach((dato,i) => { //Si se encuentra un dato de la busqueda se carga la tabla con sus respectivos campos
            if (dato.estado_plaMejor=="Abierto") {
                
            
            ruta=nombreArchivo(dato.ruta_evidencia);
            tabla += `<tr>
                <td>${i+1}</td>
                ...
                <td>
                <button type='button' class='btn btn-primary btn-sm' data-toggle='modal' data-target='#staticBackdrop1' onclick='ValidarEditar(${dato.id_plan_mejoramiento});validarEvidencia(${dato.id_plan_mejoramiento},"${ruta}")'>Editar</button>
            ...
            }else if (data.estado_plaMejor=="Cerrado"||data.estado_plaMejor=="Vencido") {
                ruta=nombreArchivo(dato.ruta_evidencia);
                tabla += `<tr>
                <td>${i+1}</td>
                <td><a data-toggle='modal' data-target='#staticBackdrop4' onclick='hallazgo(${dato.id_hallazgo})'>${dato.tema_hallazgo}</a></td>
